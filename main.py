        ...
        try:
            ...
            for selector in claim_selectors:
                try:
                    ...
                    if btn.is_visible():
                        card = btn.locator("xpath=./ancestor::div[contains(@class, 'card') or contains(@class, 'offer') or contains(@class, 'Product') or @class][2]").first
                        card.scroll_into_view_if_needed()
                        page.wait_for_timeout(1500)
                        
                        card_text = card.inner_text()
                        lines = [line.strip() for line in card_text.split("\n") if line.strip()]
                        ignore_list = ["ücretsiz", "günlük", "hediye", "bekliyor", "sa", "dk", "bonus", "alınmayı", "al", "claim"]
                        for line in lines:
